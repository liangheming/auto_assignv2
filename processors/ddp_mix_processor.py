    ...
    def train(self, epoch):
        ...
        for i, (img_tensor, targets_tensor, _) in enumerate(pbar):
            ...
            with amp.autocast(enabled=True):
                cls_outputs, reg_outputs, center_outputs, grids, gaussian = self.model(img_tensor)
            ...
            negative_loss, positive_loss = detail_loss
            loss_list[0].append(total_loss.item())
            loss_list[1].append(negative_loss.item())
            loss_list[2].append(positive_loss.item())
            if self.local_rank == 0:
                pbar.set_description(
                    "epoch:{:3d}|match_num:{:4d}|size:{:3d}|target_loss:{:6.4f}|negative_loss:{:6.4f}|positive_loss:{:6.4f}|lr:{:8.6f}".format(
                        epoch + 1,
                        int(total_num),
                        h,
                        total_loss.item(),
        ...
        print(
            "epoch:{:3d}|match_num:{:4d}|local:{:3d}|target_loss:{:6.4f}|negative_loss:{:6.4f}|positive_loss:{:6.4f}|lr:{:8.6f}"
                .format(epoch + 1,
                        int(match_num),
                        self.local_rank,
                        mean_loss_list[0],
